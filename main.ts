...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Bumper, function (sprite, otherSprite) {
    if (sprite.x > 0 && !(sprite.isHittingTile(CollisionDirection.Bottom)) || sprite.x < otherSprite.x) {
        ...
        info.changeLifeBy(-1)
        sprite.say("Quack!", invincibilityPeriod)
...
function giveIntroduction () {
    ...
    game.setDialogCursor(img`
        ...
        . . . . . . . . . . . . . . . . 
        `)
    showInstruction("Muévete con los botones \"Izquierda\" y \"Derecha en el d-pad")
    showInstruction("Salta con el botón \"A\"")
    showInstruction("Puedes hacer un doble salto presionando \"A\" en el aire")
}
...
function animateCrouch () {
    ...
    mainCrouchLeft.addAnimationFrame(assets.image`patano agachado`)
...
scene.onOverlapTile(SpriteKind.Player, assets.tile`tile1`, function (sprite, location) {
    ...
    currentLevel += 1
    if (hasNextLevel()) {
        game.splash("¡Al siguiente nivel!")
...
hero = sprites.create(assets.image`patano`, SpriteKind.Player)
